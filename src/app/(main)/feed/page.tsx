...
export default function FeedPage() {
    const { nearbyUsers, currentUser, connections, sendRequest, events } = useStore();
        ...
        <div className="container min-h-screen pb-24 pt-8 animate-fade-in">
            ...
            <header className="mb-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                <h1 className="text-xl font-bold mb-3 is-sticky text-shadow-sm transition-all">
                    {search ? "Search Spirits" : "My Spirits"}
                </h1>
            ...
            <div className="flex flex-col gap-4">
                {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((user, index) => {
                        ...
                        const activeEvent = events.find(e => e.attendees.includes(user.id));
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {/* Active Event Status */}
                                    {activeEvent ? (
                                        <div className="mt-1 flex items-center gap-1.5 text-xs text-[var(--ghost-secondary)] font-medium animate-pulse">
                                            <span>📍</span>
                                            <span className="truncate">
                                                {activeEvent.hostId === user.id ? `Hosting: ${activeEvent.title}` : `At: ${activeEvent.title}`}
                                            </span>
                                        </div>
                                    ) : (
